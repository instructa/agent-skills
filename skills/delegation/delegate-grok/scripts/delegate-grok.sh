...
usage() {
  printf '%s\n' \
    'Usage: delegate-grok.sh <review|implement> [options]' \
    '' \
    'Options:' \
    '  --task TEXT          Task text.' \
    '  --task-file PATH     Read task text from a file.' \
    '  --target TARGET      Reuse a specific Herdr agent name or pane ID.' \
    '  --target-title TITLE Reuse the Grok pane with this exact title.' \
    '  --model MODEL        Grok model used when starting Grok. Default: grok-4.6.' \
    '  --effort EFFORT      Reasoning effort. Default: xhigh.' \
    '  --timeout MS         Prompt settlement timeout. Default: 600000.' \
    '  --dry-run            Print the generated Grok prompt without using Herdr.' \
...
task=""
task_file=""
target_override="${DELEGATE_GROK_TARGET:-}"
target_title="${DELEGATE_GROK_TARGET_TITLE:-}"
model="${DELEGATE_GROK_MODEL:-grok-4.6}"
effort="${DELEGATE_GROK_EFFORT:-xhigh}"
timeout_ms="600000"
dry_run="0"
...
if [[ "$mode" == "review" ]]; then
  printf -v prompt '%s\n\nProject: %s\n\nTask:\n%s\n\nRules:\n%s\n%s\n%s\n%s\n%s\n\nReturn exactly these sections:\n%s\n%s\n%s\n%s' \
    'You are Grok acting as an independent read-only reviewer.' \
    "$project_cwd" \
    "$task" \
  ...
  printf -v prompt '%s\n\nProject: %s\n\nTask:\n%s\n\nRules:\n%s\n%s\n%s\n%s\n%s\n%s\n\nReturn exactly these sections:\n%s\n%s\n%s\n%s\n%s' \
    'You are Grok acting as the sole implementation writer for this delegated task.' \
    "$project_cwd" \
    "$task" \
...
command -v grok >/dev/null 2>&1 || fail 'grok is not installed or not on PATH'
...
if [[ -z "$target" ]]; then
  ...
  agent_name="grok-${workspace_id//[^[:alnum:]-]/-}"
  printf 'delegate-grok: starting %s in %s\n' "$agent_name" "$pane_id" >&2
  herdr agent start "$agent_name" \
    --kind grok \
    --pane "$pane_id" \
    --timeout 120000 \
    -- --model "$model" --reasoning-effort "$effort" >/dev/null
  target="$agent_name"
fi
...
printf 'delegate-grok: sending %s task to %s\n' "$mode" "$target" >&2
set +e
...
if (( prompt_exit != 0 )); then
  if grep -q 'agent_prompt_stalled' <<<"$prompt_result"; then
    printf 'delegate-grok: Grok did not submit; sending Enter fallback\n' >&2
    ...
    [[ "$state_changed" == "1" ]] || fail 'Grok still showed no state change after the Enter fallback'
    ...
    if [[ "$agent_status" == "working" ]]; then
      ...
      set -e
      if (( wait_exit != 0 )); then
        printf 'delegate-grok: settlement timed out; cancelling %s\n' "$target" >&2
        ...
        herdr agent read "$target" --source recent-unwrapped --lines 200 || \
          herdr agent read "$target" --source visible --lines 200 || true
        fail 'Grok exceeded the settlement timeout'
      fi
    fi
  elif grep -qiE 'timed out|"code":"timeout"|code: timeout' <<<"$prompt_result"; then
    printf 'delegate-grok: prompt timed out; cancelling %s\n' "$target" >&2
    herdr agent send-keys "$target" esc >/dev/null 2>&1 || true
    herdr agent wait "$target" --until idle --until "done" --until blocked \
      --timeout 10000 >/dev/null 2>&1 || true
    printf '%s\n' "$prompt_result" >&2
    herdr agent read "$target" --source recent-unwrapped --lines 200 || \
      herdr agent read "$target" --source visible --lines 200 || true
    fail 'Grok exceeded the prompt timeout'
  else
    printf '%s\n' "$prompt_result" >&2
    fail 'Herdr could not submit or settle the Grok task'
  fi
fi
...
if [[ "$agent_status" == "blocked" ]]; then
  printf 'delegate-grok: Grok is blocked; do not auto-approve the pending action\n' >&2
  exit 3
fi

[[ "$agent_status" == "idle" || "$agent_status" == "done" ]] || fail "Grok ended in unexpected state: $agent_status"

if [[ "$mode" == "review" ]]; then
  for section in STATUS FINDINGS EVIDENCE RECOMMENDATION; do
    grep -Eq "^[[:space:]]*${section}:" <<<"$output" ||
      fail "Grok returned an incomplete handoff: missing ${section}"
  done
else
